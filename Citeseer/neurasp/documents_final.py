...
for seed in range(0, 10):
    ...
    dataList_train = []
    obsList_train = []
    for doc, index, label in trainDataset:
        dataList_train.append({'doc': doc.unsqueeze(0), 'ind': index})
        obsList_train.append(f':- not document_label(ind, {label}).')

    dataList_val = []
    obsList_val = []
    for doc, index, label in valDataset:
        dataList_val.append({'doc': doc.unsqueeze(0), 'ind': index})
        obsList_val.append(f':- not document_label(ind, {label}).')

    dataList_test = []
    obsList_test = []
    for doc, index, label in testDataset:
        dataList_test.append({'doc': doc.unsqueeze(0), 'ind': index})
        obsList_test.append(f':- not document_label(ind, {label}).')
