...
def read_triples_mnist(path: str) -> List[Tuple[str, str, str]]:
    triples = []
    with open(path, 'rt') as f:
        for line in f.readlines():
            ...
            if 'train' in path:
                ...
                used_dataset = 'test'
            image_1 = datasets[used_dataset][int(s.strip())][0].numpy().flatten()
            image_2 = datasets[used_dataset][int(o.strip())][0].numpy().flatten()
            triples += [(str(image_1.data), p.strip(), str(image_2.data))]
    return triples
