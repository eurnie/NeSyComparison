...
for batch_size in [2, 4, 8, 16, 32, 64]:
    ...
    for index_1, doc_1, label_1 in trainDataset:
        for index_2, doc_2, label_2 in trainDataset:
            if index_1 != index_2:
                for i in range(len(cites_a)):
                    if index_1 == cites_a[i] and index_2 == cites_b[i]:
                        dataList_train.append({'doc_1': doc_1, 'doc_2': doc_2})
                        obsList_train.append(f':- not document_label(doc_1, doc_2, {label_1}, {label_2}, 1).')
                        break

                dataList_train.append({'doc_1': doc_1, 'doc_2': doc_2})
                obsList_train.append(f':- not document_label(doc_1, doc_2, {label_1}, {label_2}, 0).')
