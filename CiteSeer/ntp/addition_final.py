...
for seed in range(0, 1):
    ...
    generate_dataset(dataset, to_unsupervised, seed)

    conf = load_conf("default.conf")
    experiment_prefix = "citeseer"
    ...
    kb = load_from_file('data/citeseer.nl')
    ...
    training_time = start_training_time - time.time()
    print('--------- START EVALUATION ---------')
    start_testing_time = time.time()
    test_set = "data/val.txt"
    ...
    with open(test_set, "r") as f:
        for line in f.readlines():
            ...
            test_x.append(line_splitted[0])
            test_y.append(int(line_splitted[1][:-1]))

    nb_possible_results = 6
    ...
    predicate_ids = [vocab('document_label')] * nb_possible_results
    possible_labels = ['0', '1', '2', '3', '4', '5']
    possible_labels_ids = [vocab(x) for x in possible_labels]

    def predict(document_id):
        feed_dict = {}

        document_ids = [vocab(document_id)] * nb_possible_results
        goal = [predicate_ids, document_ids, possible_labels_ids]
    ...
    testing_time = start_testing_time - time.time()
